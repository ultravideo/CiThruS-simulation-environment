Shader "LensEffects/CameraLensRainDrop"
{
	...
	Properties
	{
		...
		_RainWhiteness("_RainWhiteness", Float) = 1
		[Toggle] _DoRain("do rain", Float) = 0
	}

	SubShader
	{
		Cull Off ZWrite Off ZTest Always //Blend SrcAlpha SrcAlpha
		...
		CGPROGRAM
			...
			fixed4 frag(v2f_img i) : COLOR
			{
				...
				fixed4 rTex = tex2D(_MainTex, i.uv);
				...
				mask = mask * (1 - isMask);
				//rTex = mainTex * isMask + (1 - mask) * gaussian_filter(mask, i.uv, float2(_MainTex_TexelSize.x, 0), _MainTex) * (1 - isMask);

				rTex = mainTex * isMask + (1 - mask) * gaussian_filter(mask, i.uv, float2(_MainTex_TexelSize.x, 0), _MainTex) * (1 - isMask) * _RainWhiteness;
